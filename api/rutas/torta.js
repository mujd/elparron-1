...
REST_ROUTER.prototype.handleRoutes = function(router, connection, md5) {
    ...
    router.get("/torta/precio/:masaTipo_id/:masaSabor_id/:sabor_id/:tamano_id", function(req, res){
        connection.query(`
            SELECT pre.id, pre.precio 
            FROM precio AS pre 
            INNER JOIN tortaPrecio AS pretor ON pre.id = pretor.precio_id 
            INNER JOIN torta AS tor ON tor.id = pretor.torta_id 
            INNER JOIN tamano AS tam ON tam.id = pretor.tamano_id 
            WHERE   tor.masaTipo_id = ` + req.params.masaTipo_id + ` AND 
                    tor.masaSabor_id = ` + req.params.masaSabor_id + ` AND 
                    tor.sabor_id = ` + req.params.sabor_id + ` AND 
                    tam.id = ` + req.params.tamano_id, function(err, rows) {
            if(err){
                res.json({"error": err});
            }else{
                res.json(rows);
